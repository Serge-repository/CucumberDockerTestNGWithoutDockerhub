    ...
    stages {
        ...
        stage('Execute tests'){
            steps {
                script {
                    if ( !TAGS.isEmpty() ) {
                        bat "mvn clean test -Dcucumber.filter.tags=${TAGS} -Dbrowser=${params.browser} -DremoteLocal='remote' -Dforks=${params.forks} org.testng.TestNG"
                    }
                    if ( !FEATURE_CLASS.isEmpty() ) {
                        bat "mvn clean test -Dcucumber.options=${FEATURE_CLASS} -Dbrowser=${params.browser} -DremoteLocal='remote' -Dforks=${params.forks} org.testng.TestNG"
                    }
                }
            }
        }
